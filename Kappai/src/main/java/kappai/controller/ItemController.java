...
public class ItemController {
	...
	// Method to delete an item from the item master list based on a JSON request
	private static String callDeleteItem(String jsonInput) {
		// Initialize the variable to store the JSON response
		String jsonResponse = null;

    	try {
    		// Step 1: JSON Deserialization - Convert the JSON string input into a Java object
            
            // The input JSON string is first converted to a stream of bytes
			InputStream jis = new ByteArrayInputStream(jsonInput.getBytes());
			// A JsonReader is created to read the JSON from the input stream
			JsonReader jsonReader = Json.createReader(jis);
			 // The JsonObject is built from the input JSON string, allowing access to individual fields
			JsonObject jsonObject = jsonReader.readObject();
			 // Close the JsonReader and input stream to free resources
			jsonReader.close();
			jis.close();
			// Log the extracted itemID from the input JSON for debugging
			System.out.println("callDeleteItem itemID="+jsonObject.getString("itemID"));
			
			 // Step 2: Set up the Value Object (VO) based on the extracted JSON data
	        
	        // Create an instance of the ItemDAO to interact with the database
	        ItemDAO itemDAO = new ItemDAO();
	        // Create a new ItemVO object to represent the item being deleted
	        ItemVO itemVOInput = new ItemVO();
	        
	        
	        // Extract the itemID from the JSON and set it in the ItemVO
	        itemVOInput.setItemID(jsonObject.getString("itemID"));
	        
	        // Step 3: Call the DAO method to delete the item and handle the result
	        
	        // Call the DAO to delete the item from the database based on the itemID
	        int noOfRecordsDeleted = itemDAO.deleteItem(itemVOInput);
	        
	        // Log the result of the deletion for debugging
	        System.out.println("callDeleteItem noOfRecordsDeleted="+noOfRecordsDeleted);
	        
	        // Step 4: JSON Serialization - Build the response JSON based on the outcome of the deletion
	        if (noOfRecordsDeleted==1) {
	            // If the deletion was successful, return a success message in JSON format
	        	jsonResponse ="{\"status\":\"success\", \"itemID\":\""+itemVOInput.getItemID()+ "\"}";
	        }
	        else {
	            // If the deletion failed, return an error message in JSON format
	        	jsonResponse ="{\"status\":\"error\"}";
	        	
	        }
	        
		} catch (IOException e) {
	        // Handle any IO exceptions that occur during JSON deserialization or serialization
			e.printStackTrace();
		}
    	
        // Return the generated JSON response back to the caller
    	return jsonResponse;
	}
...
private static String callAddItem(String jsonInput) {
    	...
    	try {
			...
			System.out.println("itemID="+jsonObject.getString("itemID"));
